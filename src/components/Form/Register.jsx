...
export default function RegisterForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        const response = await axios.post("https://65a25d5342ecd7d7f0a771bd.mockapi.io/users", formData);
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="mx-auto my-8 max-w-sm">
        ...
        {/* Submit button */}
        <button type="submit" className="w-full rounded-lg bg-blue-500 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-blue-700 focus:ring-4 focus:ring-blue-300">
          Submit
        </button>
      </form>
    </>
